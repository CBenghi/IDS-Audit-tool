	...
	internal class IdsRepo_Updater
	{
		...
		public static IEnumerable<UpdatableFile> GetAllUpdatable(DirectoryInfo solutionDir)
		{
			return UpdateDocumentationUnits(solutionDir)
				...
				.Concat(UpdateEntityMappingTypes(solutionDir))
				.Concat(UpdateTestingSchema(solutionDir));
		}

		private static IEnumerable<UpdatableFile> UpdateEntityMappingTypes(DirectoryInfo solutionDir)
		{
			var source = Path.Combine(
					solutionDir.FullName,
					"ids-lib.codegen",
					"buildingSMART",
					"ifc2x3-occurrence-type-mapping-table.md"
					);
			var sourceFile = BuildingSmartRepoFiles.GetDocumentation("ifc2x3-occurrence-type-mapping-table.md", "ImplementersDocumentation");
			var destFile = new FileInfo(source);
			if (sourceFile.Exists)
			{
				if (BuildingSmartRepoFiles.FilesAreIdentical(sourceFile, destFile))
					yield break;
				yield return new UpdatableFile("IFC2X3 occurrence to entity/type mapping", sourceFile.FullName, destFile.FullName);
			}
		}
